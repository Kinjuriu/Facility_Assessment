...
# DATA CLEANING
facility.replace(
    "---", np.nan, regex=False, inplace=True
)  # replace the dashes with Nan
print(facility.isin(["---"]).sum().sum())  # check whether the dashes are gone

facility_df = facility.replace(
...
df = facility_df.dropna(axis=1, thresh=min_count)
print(df.head())
...
# We will get a summary statistic for numerical columns only
print(df.describe())
...
df.drop(
    "form.sections_to_review", axis=1, inplace=True
)  # dropping column form sections to review
column_types = df.dtypes  # check the type of columns

# convert object columns to float
# first range [5, 25]
object_to_float_1 = df.iloc[:, 5:25].astype(float)
# second range  [27, 42]
object_to_float_2 = df.iloc[:, 27:42].astype(float)
# second range  [145, 161]
object_to_float_3 = df.iloc[:, 145:161].astype(float)

# add two df's together
float_cols = pd.concat(
    [object_to_float_1, object_to_float_2, object_to_float_3], axis=1
)
...
get_dummy_data = pd.concat(
    [dataframe_1, dataframe_2], axis=1
)  # add both df's since the columns were in different locations


# Creating dummy variables for the categorical columns
dummy_data = pd.get_dummies(
    get_dummy_data,
    columns=get_dummy_data.select_dtypes(
...
# NB // Lat/Long coordinates can be used as-is with tree-based models
# that do not require data to be normalized.
# Neural networks usually require those coordinates to be normalized:
...
# Elbow curve, the point after which inertia starts decreasing in a linearly
...
labels = kmeans.predict(
    coordinates_data[coordinates_data.columns[1:3]]
...
# Combine the data that has been transformed except
# ordinal_encoded_data and scaled_numerical_data are in numpy.array
